package br.com.bonfimsoft.certsreader;
...
public class CertsReader {
    ...
    public static File gerarCacerts(String host, int port) throws Exception {
        ...
        File file = new File(EXPORTED_CACERTS);
        if (file.isFile() == false) {
            char SEP = File.separatorChar;
            File dir = new File(System.getProperty("java.home") + SEP + "lib" + SEP + "security");
            file = new File(dir, EXPORTED_CACERTS);
            if (file.isFile() == false) {
        ...
        for (int i = 0; i < chain.length; i++) {
            ...
            info("| Added certificate to keystore '" + EXPORTED_CACERTS + "' using alias '" + alias + "'");
        }

        File cafile = new File(EXPORTED_CACERTS);
